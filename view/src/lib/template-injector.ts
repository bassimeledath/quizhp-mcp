...
function appendPostMessageShim(html: string): string {
  const shim = [
    ...
    "  function getEngine(){",
    "    try {",
    "      if (window.engine && window.engine.end) return window.engine;",
    "    } catch (e) {}",
    "    try {",
    "      if (typeof engine !== 'undefined' && engine && engine.end) return engine;",
    "    } catch (e) {}",
    "    return null;",
    "  }",
    ...
    "    try {",
    "      var targetEngine = getEngine();",
    "      if (targetEngine && targetEngine.end && !targetEngine.__wrapped){",
    "        var _end = targetEngine.end.bind(targetEngine);",
    "        targetEngine.end = function(ok, explanation){",
    "          try { post('quiz-end', { isCorrect: !!ok, explanation: explanation || '' }); } catch(e){}",
    "          return _end(ok, explanation);",
    "        };",
    "        targetEngine.__wrapped = true;",
    "        try { window.engine = targetEngine; } catch (e) {}",
